...
- Original
   Source availabe at: https://github.com/cosmo0920/win32-api; https://rubygems.org/gems/win32-api

- Modifications
   Add monkey-patching C code in order to install `SetConsoleCtrlHandler` API, which fails by Ruby implementation due to stack overflow issues (not yet clear on the exact cause)
...
//////////////////// Modified codes: SetConsoleCtrlHandler callback function
DWORD main_thread;
static BOOL WINAPI CtrlHandler(DWORD fdwCtrlType) {
  switch (fdwCtrlType) {
    ...
    case CTRL_CLOSE_EVENT: // CTRL-CLOSE: confirm that the user wants to exit
        Beep(900, 200);
        PostThreadMessage(main_thread, WM_APP, 1, 0); // inform the Ruby main
// HACK: THIS IS EVIL!
// in order to let the process to stay there for a while rather than ungracefully exit within 5s. refs:
// https://stackoverflow.com/a/18445043
// https://stackoverflow.com/a/21504556
// https://github.com/FarGroup/FarManager/blob/a2dcfa794061582725acbce06676351e711028c3/far/interf.cpp#L215
        ExitThread(1);
        return FALSE;
    default:
        return FALSE;
...
//////////////////// Modified codes: Monkey-patching for SetConsoleCtrlHandler: take over this API call only
  VALUE v_func = rb_iv_get(self, "@function_name");
  if(strcmp(RSTRING_PTR(v_func), "SetConsoleCtrlHandler") == 0) {
    main_thread = GetCurrentThreadId();
//char id[10];
//itoa(main_thread, id, 10);
//MessageBox(NULL, id, "test", 0);
    return_value = SetConsoleCtrlHandler(CtrlHandler, TRUE);
