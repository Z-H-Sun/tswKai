...
class Rev
  ...
  @@oldbyte = [["\x7C\xF4\x47\x00\x09speedhigh", "\x64", "\x74\xF4",
    ...
    ["\x8B\x83\xA8\x03\x00\x00\x80\x78\x28\x01", "",
  ...
  @@newbyte = [["\x64\xF4\x47\x00\x09DisTEdit8", "\x7C", "\x44\xF5",
    ...
    ["\xC1\xEE\x02\xBD\x04\x00\x00\x00\xEB\x29", "Sleep\0",
    ...
    ["\x31\xC0\x8A\x16\x80\xEA\x25\x80\xFA\x03\x76\x21\x39\x05\xAC\xC5\x48\x00\x0F\x8F\xA9\x04\x00\x00\xA3\xAC\xC5\x48\x00\x8B\x83\x28\x03\x00\x00\x80\x78\x29\x01\x0F\x85\x94\x04\x00\x00", [0xe9, 0x104, # jmp 460d78
      120, 120, 150, 225, # interval1
      0, 10, 100, 175, # interval2 (-50)
      75, 75, 150, 225, # interval0 (-50)
  ...
  def patch(bytes, offset, offset0=-0xC00)
    @f.seek(offset+offset0)
    @f.write(bytes ? bytes
      : (@c ? "\x08Î¢ÈíÑÅºÚ" : "\x07Verdana")) # 'Microsoft YaHei'
  end
  def rev(index, closeFile=true)
    @@address[index].each_with_index {|x, i| patch(@@newbyte[index][i], x)}
    @f.close() if closeFile
  end
  def res(index, closeFile=true)
    @@address[index].each_with_index {|x, i| patch(@@oldbyte[index][i], x)}
    @f.close() if closeFile
  end
  def revAll()
    (0...@@address.size).each {|i| rev(i, false)}
    @f.close()
  end
  def resAll()
    (0...@@address.size).each {|i| res(i, false)}
    @f.close()
  end
end

Rev.new('TSW.exe').revAll()
Rev.new('TSW.EN.exe').revAll()
Rev.new('TSW.CN.exe', true).revAll()
Rev.new('TSW.CNJP.exe', true).revAll()
system('pause')
