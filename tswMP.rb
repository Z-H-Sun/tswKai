...
# - @winDown [nil or Boolean]: State of MP_KEY1(by default, left WIN)/MP_KEY2(by default, TAB) keys [`nil` if not pressed, `false` if pressed but within an event (so no need to respond), `true` if pressed and active].
...
# - drawMapDmg(init [Boolean]): After an event is over, updates game map and draws map damage overlay. The parameter `init` is only used for re-checking the map, and indicates whether all properties of monsters and magic attacks should be recalculated.
# - drawDmg(x, y [Integer], dmg [String], cri [nil or false or String], danger [Boolean]): Draws damage text on map. `cri` is `nil` if no further critical value; `false` if magic attack; or a string if normal critical value. `danger` indicates whether the damage is may cause death.
# - drawItemsBar: Highlights items and labels their corresponding hotkeys.
...
# - _keyHook(nCode, wParam, lParam): Keyboard hook callback; for item usage and tswMP function activation. Return value dictates whether to block the keyboard input.
...
# - noHighlightRect: Clears item/message bar drawings.
...
# Drawing functions below. In most cases, call aliases `showMsg` and `showMsgTxtbox`, unless there is a special need to specify which of the "A" or "W" version should be used
# `showMsg`: draws the specified, formatted texts in the message prompt region with a solid-color background behind the text; `showMsgTxtbox`: shows texts in the status bar textbox
